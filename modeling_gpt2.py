...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, K=32):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, K=32):
        super().__init__()
        nx = config.n_embd
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = Attention(nx, n_ctx, config, scale, K=K)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, K=config.K) for _ in range(config.n_layer)])
